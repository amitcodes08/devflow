...
import { getDeviconClassName, getTechDescription, cn } from '@/lib/utils';
...
const TagCard = ({_id, name, questions, showCount, compact, remove, isButton, handleRemove} : Props) => {
  ...
  const handleClick= (e : React.MouseEvent) => {
    ...
    const Content = (
      <>
        <Badge className="subtle-medium background-light800_dark300 text-light400_light500 rounded-md border-none px-4 py-2 uppercase flex flex-row gap-2">
          <div className="flex-center spacce-x-2">
            <i className={`${iconClass} text-sm p-2`}></i>
            <span>{name}</span>
          </div>
          {remove && (
    ...
    if(compact) {
        return isButton ? (
            <button onClick={handleClick} className='flex-between gap-2'>{Content}</button>
        ) : (
